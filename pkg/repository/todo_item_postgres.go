...
func (r *TodoItemPostgres) Create(listId int, item todo.TodoItem) (int, error) {
	...
	var itemId int
	createItemQuerty := fmt.Sprintf("INSERT INTO %s (title,description) VALUES ($1,$2) RETURNING id", todoItemsTable)
	row := tx.QueryRow(createItemQuerty, item.Title, item.Description)
	err = row.Scan(&itemId)
	if err != nil {
		tx.Rollback()
		return 0, err
	}
	createListItemQuerty := fmt.Sprintf("INSERT INTO %s (list_id,item_id) VALUES ($1,$2)", listsItemsTable)
	_, err = tx.Exec(createListItemQuerty, listId, itemId)
	if err != nil {
		tx.Rollback()
		return 0, err
	}
...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]todo.TodoItem, error) {
	...
	li.item_id=ti.id INNER JOIN %s ul on ul.list_id=li.list_id WHERE li.list_id=$1 AND ul.user_id =$2`,
...
func (r *TodoItemPostgres) GetById(userId, itemId int) (todo.TodoItem, error) {
	...
	query := fmt.Sprintf(`SELECT  ti.id,ti.title,ti.description,ti.done  FROM %s ti INNER JOIN %s li on 
	li.item_id=ti.id INNER JOIN %s ul on ul.list_id=li.list_id WHERE ti.id=$1 AND ul.user_id =$2`,
		todoItemsTable, listsItemsTable, usersListsTable)
